// Generado por scripts/extraer-css-dc.mjs desde design/panel-operador/dc/Panel Operadora.html
// (los bloques <style> 2 a 4 del template desempacado; el bloque 1 es la hoja base del bundler
// de Claude Design —h1, p, svg…— que admin-css ya cubre y NO se copia: restilaría todo el panel).
// Antes venía de «Operador Mi Alta v5», que no traía los acordeones del expediente (.actv/.hers).
...
.adm .clogo{height:22px;width:auto;display:block}
.adm .camsw{display:inline-flex;align-items:center;padding:4px 10px;border:1px dashed var(--sand);border-radius:999px;font-size:12px;white-space:nowrap}
.adm .camsw *{white-space:nowrap}
.adm .opt{white-space:nowrap}
.adm .cambar{display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin:0 22px 12px;padding:11px 15px;border-radius:12px;background:var(--charcoal);color:rgba(255,255,255,.78);font-size:12.5px;line-height:1.55}
.adm .cambar .g{flex:1 1 360px}
.adm .cambar b{color:#fff;font-weight:600}
.adm .camrow{grid-column:1/-1;display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:8px;padding:9px 12px;border-radius:10px;background:rgba(32,33,28,.06);border:1px dashed rgba(32,33,28,.25)}
.adm .camrow .lb{font-size:9.5px;letter-spacing:.14em;text-transform:uppercase;font-weight:700;color:var(--charcoal);margin-right:4px}
.adm .camrow .ac{display:flex;gap:8px;flex-wrap:wrap}
.adm .camrow>div:not(.ac){flex:1 1 100%}
.adm .doc .ac{align-items:center}
.adm .nav .fut.onsec{background:var(--charcoal);color:#fff;border-color:var(--charcoal)}
.adm .nav .fut.onsec .dt{background:#fff;border-color:#fff}
...
.adm .frm{display:flex;flex-direction:column;gap:12px}
.adm .sel{display:flex;flex-direction:column;gap:6px;font-size:12.5px;font-weight:500;color:var(--charcoal)}
.adm .sel select{font:inherit;font-weight:400;font-size:14px;padding:10px 12px;border:1px solid var(--line);border-radius:12px;background:#fff;color:var(--charcoal)}
.adm .sel select:disabled{opacity:.55}
.adm .cks{display:flex;flex-direction:column;gap:10px;margin-top:14px}
.adm .cati.sel{border-color:var(--orange);box-shadow:0 0 0 3px rgba(255,93,54,.12)}
.adm .opt:disabled{opacity:.55;cursor:not-allowed}
.adm .nowcard{min-width:0}
.adm .mitad .mb .embed{padding:12px}
@media(max-width:640px){.adm .cambar{margin:0 12px 10px}}
.adm a:hover{color:var(--orange)}
`;
