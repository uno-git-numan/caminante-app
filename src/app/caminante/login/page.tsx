...
import { sendMagicLink, signInWithGoogle, signInWithApple, signInWithPasswordAction } from "@/lib/auth/actions";
...
export default async function LoginPage({ searchParams }: LoginPageProps) {
  const { sent, email, error, next, mode } = await searchParams;
  const nextPath = next?.startsWith("/") ? next : "/caminante";
  const isPassword = mode === "password";

  const magicTabHref = nextPath !== "/caminante"
    ? `/caminante/login?next=${encodeURIComponent(nextPath)}`
    : "/caminante/login";
  const passwordTabHref = nextPath !== "/caminante"
    ? `/caminante/login?mode=password&next=${encodeURIComponent(nextPath)}`
    : "/caminante/login?mode=password";
