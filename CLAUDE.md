...
## Perfiles (roles) + login multi-método (29 jun — EN PREVIEW, falta config Google + promote)
...
- **Login (`/caminante/login`) y signup (`/caminante/signup`)** ahora ofrecen 3 métodos: **Google OAuth** (`GoogleButton.tsx` client → `signInWithOAuth` → callback PKCE en `src/app/caminante/auth/callback/route.ts` → `exchangeCodeForSession`), **email+contraseña** (`signInWithPassword`/`signUpWithPassword` en `actions.ts`), y el **magic link** de siempre. Re-skin a la marca (voz "tú", sin emoji). El panel admin (`admin/page.tsx`) quedó con tarjetas etiquetadas.
- ⚠️ **PENDIENTE de Luis para que Google funcione (no lo puede hacer Claude — credenciales):** (1) Google Cloud Console → crear OAuth Client ID (Web), redirect URI = `https://hnyoahirxmzkshivgvnm.supabase.co/auth/v1/callback`; (2) Supabase → Authentication → Providers → Google → ON + pegar Client ID/Secret; (3) Supabase → URL Configuration → agregar `…/caminante/auth/callback` a la allowlist (con wildcards de prod/preview/localhost). Hasta entonces el botón de Google muestra error al hacer clic; magic link y contraseña SÍ funcionan. **Por eso el tip NO está promovido a prod** — promover tras configurar Google.
